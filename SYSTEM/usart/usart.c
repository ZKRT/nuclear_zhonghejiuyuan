...
{ 	
	while((USART1->SR&0X40)==0);//循环发送,直到发送完毕   //zkrt_debug
	USART1->DR = (u8) ch;      
	return ch;
}
#endif
...
//	GPIO_Init(GPIOA,&GPIO_InitStructure); //初始化PA9，PB7
//	
//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;
//	GPIO_Init(GPIOB,&GPIO_InitStructure); //初始化PA9，PB7
...
//	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=50;//抢占优先级3
//	NVIC_InitStructure.NVIC_IRQChannelSubPriority =50;		//子优先级3
...
//	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
//	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
//	GPIO_Init(GPIOC,&GPIO_InitStructure); //初始化PA9，PB7
//	
//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;
//	GPIO_Init(GPIOC,&GPIO_InitStructure); //初始化PA9，PB7
...
//	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;//无硬件数据流控制
//	USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;	//收发模式
//  USART_Init(API_USART, &USART_InitStructure); //初始化串口1
//	
//  USART_Cmd(API_USART, ENABLE);  //使能串口1 
//	
//	USART_ClearFlag(API_USART, USART_FLAG_TC);
//	
//	USART_ITConfig(API_USART, USART_IT_RXNE, ENABLE);//开启相关中断
...
//	OSIntEnter();    
//#endif
//	if(USART_GetITStatus(API_USART, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
//	{
//		Res =USART_ReceiveData(API_USART);//(USART1->DR);	//读取接收到的数据
//		
...
//	{
//	  while(USART_GetFlagStatus(API_USART,USART_FLAG_TC)==RESET)
//		{}			//等待发送结束		
//    USART_SendData(API_USART,buf[t]); //发送数据
//	}
//	while(USART_GetFlagStatus(API_USART,USART_FLAG_TC)==RESET); //等待发送结束		
...
static void uart_gpio_init(void) {
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = NVIC_PPRIORITY_U1;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = NVIC_PPRIORITY_U1;
